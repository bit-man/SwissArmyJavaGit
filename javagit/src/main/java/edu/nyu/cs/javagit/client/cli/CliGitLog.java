...
public class CliGitLog implements IGitLog{
	...
	/**
	 * This function build the git log commands wih necessary options as specified by the user.
	...
	public class GitLogParser implements IParser {
		...
		public void parseLine(String line) {
			...
			if (line.startsWith("commit")){
				...
				canCommit = true;
				tmp = line.split(" ");
				response.setSha(line.substring(tmp[0].length()));
			}
			...
			//Author
				...
				tmp = line.split(" ");
				response.setAuthor(line.substring(tmp[0].length()));
			}

			//Date
			else if (line.startsWith("Date")){
				tmp = line.split(" ");
				response.setDate(line.substring(tmp[0].length()));
			}
			//message or fileDetails
			else{
